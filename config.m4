...
if test "$PHP_MONGODB" != "no"; then
  ...
  if test "$PHP_MONGODB_SYSTEM_LIBS" != "no"; then
    ...
    if test -x "$PKG_CONFIG" && $PKG_CONFIG --exists libbson-1.0; then
      if $PKG_CONFIG libbson-1.0 --atleast-version 1.23.2; then
        ...
        AC_MSG_ERROR(system libbson must be upgraded to version >= 1.23.2)
      fi
    else
    ...
    if test -x "$PKG_CONFIG" && $PKG_CONFIG --exists libmongoc-1.0; then
      if $PKG_CONFIG libmongoc-1.0 --atleast-version 1.23.2; then
        ...
        AC_MSG_ERROR(system libmongoc must be upgraded to version >= 1.23.2)
      fi
    else
