...
if test "$PHP_MONGODB" != "no"; then
  ...
  if test "$PHP_MONGODB_SYSTEM_LIBS" != "no"; then
    PKG_CHECK_MODULES([PHP_MONGODB_BSON], [libbson-1.0 >= 1.30.7], [
      ...
      AC_MSG_ERROR([Could not find system library for libbson >= 1.30.7])
    ])

    PKG_CHECK_MODULES([PHP_MONGODB_MONGOC], [libmongoc-1.0 >= 1.30.7], [
      PHP_MONGODB_BSON_VERSION=`$PKG_CONFIG libbson-1.0 --modversion`
      PHP_MONGODB_BSON_VERSION_STRING="System ($PHP_MONGODB_BSON_VERSION)"
      ...
      AC_MSG_ERROR(Could not find system library for libmongoc >= 1.30.7)
    ])
