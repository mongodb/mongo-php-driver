--TEST--
MongoDB\Driver\BulkWriteCommand::updateOne() with invalid update document
--FILE--
<?php
...
echo throws(function() use ($bulk) {
    $bulk->updateOne(NS, ['x' => 1], ['$set' => ['x' => ['' => 1]]]);
}, 'MongoDB\Driver\Exception\InvalidArgumentException'), "\n\n";

// @TODO: FAILED: Expected MongoDB\Driver\Exception\InvalidArgumentException, but no exception thrown!
echo throws(function() use ($bulk) {
    $bulk->updateOne(NS, ['x' => 1], ['$set' => ['x' => ["\xc3\x28" => 1]]]);
}, 'MongoDB\Driver\Exception\InvalidArgumentException'), "\n\n";
...
// @TODO: FAILED: Expected MongoDB\Driver\Exception\InvalidArgumentException, but no exception thrown!
echo throws(function() use ($bulk) {
    $bulk->updateOne(NS, ['x' => 1], ['$set' => ['y' => 1], 'z' => 1]);
}, 'MongoDB\Driver\Exception\InvalidArgumentException'), "\n";
