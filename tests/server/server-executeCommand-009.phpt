--TEST--
MongoDB\Driver\Server::executeCommand() pins transaction to server
--SKIPIF--
...
$selectedServer->executeCommand(DATABASE_NAME, $command, ['session' => $session]);
...
echo throws(function () use ($wrongServer, $session) {
    ...
    $wrongServer->executeCommand(DATABASE_NAME, $command, ['session' => $session]);
